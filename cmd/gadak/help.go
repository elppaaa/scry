...
// KindStandalone with no site or credential (initBuiltIn, flag usage
...
// startServeLoops: cfg.HasCredential() is true for a built-in workspace,
...
// and admits a built-in workspace with no token. Verb --help first lines
...
var helps = map[string]cmdHelp{
	"init": {
		...
		options: []helpOption{
			...
			{name: "token-file", desc: initUsageTokenFile},
			{name: "token-stdin", desc: initUsageTokenStdin},
			{name: "token-expires", desc: initUsageTokenExpires},
			{name: "token", desc: initUsageToken},
			{name: "pairing-code", desc: initUsagePairingCode + "; binds this fresh workspace to that serve as its origin (verified against the serve before anything is saved)"},
			{name: "pairing-code-stdin", desc: initUsagePairingCodeStdin + " (keeps the secret out of ps and shell history)"},
			{name: "json", desc: initUsageJSON},
		},
		examples: []string{
			"gadak init",
			"gadak init --local",
	...
	"workspaces": {
		...
		options: []helpOption{
			{name: "json", desc: "emit JSON"},
			{name: "yes", desc: "with rm: remove — without it, rm explains and refuses"},
			{name: "destroy-origin", desc: "with rm on a built-in workspace: also delete its persist, the only copy of that tracker"},
		},
		examples: []string{
			"gadak workspaces",
	...
	"migrate": {
		...
		examples: []string{
			...
			"gadak --workspace acme migrate --from local --to jira --project ENG --dry-run",
		},
