<script lang="ts">
  ...
  import { installUiFocusPoll } from './lib/ui-focus-poll'
  ...
  onMount(() => {
    ...
    // The 500ms UI-focus poll (GDK-791/1170/960, GDK-793 dashboard watch)
    // lives in lib/ui-focus-poll.ts — one cadence, three named jobs.
    const uninstallFocusPoll = installUiFocusPoll()
    return () => {
      unbindPalette()
      unbindShortcuts()
      uninstallFocusPoll()
