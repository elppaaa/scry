...
type server struct {
	...
	// originH is the built-in issuetap handler this process already
	// holds. Lazy: first /api/v1/origin/ request constructs it via
	// origin.BuiltInHandler. Tests pin it to keep a session alive
...
func newServer(db *store.DB, cfg *config.Config, cache *attachcache.Cache, profile string) *Handler {
	...
	// Artifact bytes: same serving core, artifact response policy
	// — an HTML attachment rendered inline, opaque-origin wherever it opens.
	mux.HandleFunc("GET "+apiBase+"{key}/attachments/{id}/artifact/{$}", s.handleArtifact)
	...
	mux.HandleFunc("POST "+apiBase+"onboarding/standalone/{$}", s.handleBuiltInInit)
	...
	mux.HandleFunc("GET "+apiBase+"create-meta/fields/{$}", s.handleCreateFields)
