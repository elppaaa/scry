...
// workspace, issuetap's on a built-in one. Rows live and die with the
...
// Built-in / issuetap always fetches; Cloud keeps the DevStatus opt-in.
...
// schemaV34 caches the origin's status catalog. The changelog stores bare
// status ids, and the id -> category map lived only in each sync pass's
// memory (Batch.Categories), so a history walk could only resolve the ids
// that happen to be some issue's *current* status (GDK-591: an issue
// claimed then finished left its in-progress id unresolvable, and the
// wait/progress spans silently empty). This is origin reference data —
// rebuilt by the next sync, never time-in-status values, which
// data-model.md keeps deliberately absent.
const schemaV34 = `
...
// rebuild on upgrade. Built-in only: a connected workspace's language is
...
// origin's spelling ("agent" for built-in issuetap actors, "app" for Cloud
...
// schemaV41 (GDK-1258) gives the intuitive name to the right relation: the
// physical Jira projection becomes issues_raw (internal — writes only, never
// documented as a query surface), and `issues` is now the items-joined view
// that used to be issues_full, so SELECT key, summary, status FROM issues is
// correct by construction. issues_full stays as a compatibility alias — it
// is one of the three 0.x promises (data-model.md). The old view is dropped
// before the rename so SQLite's RENAME does not rewrite its body first.
// NOTE for the next column migration: BOTH views expand their * at CREATE
// VIEW time (the v32 rule) — an ALTER TABLE issues_raw ADD COLUMN must
// recreate `issues` AND `issues_full`, in that order.
const schemaV41 = `
DROP VIEW issues_full;
ALTER TABLE issues RENAME TO issues_raw;
...
// schemaV44 (GDK-1400) gives sync_state the two facts the field diagnosis
// needed and could not get. scope_hash is the configured scope this source
// last mirrored under: the incremental floor is one watermark per source_id,
// not per project, so widening the scope leaves the new project's existing
// issues behind a cursor that already passed them. Comparing the signature on
// the store side catches every path that widens scope — `config set projects`,
// a hand-edited config.json, a copied workspace — instead of one command.
// NULL (a pre-v44 mirror) reads as "unrecorded" and forces nothing; the first
// pass after the upgrade records it.
//
// The reconcile_* columns are the last two-way reconcile's tally. They exist
// so `gadak status` can answer "does the mirror hold what the origin holds?"
// without a full pass — the question that took a second machine to answer in
// the field.
const schemaV44 = `
ALTER TABLE sync_state ADD COLUMN scope_hash TEXT;
ALTER TABLE sync_state ADD COLUMN reconcile_at TEXT;
...
// schemaV49 adds no column: it recomputes the reopen triple
// (reopen_count / reopened_at / reopen_reason) under the rule that grew an
// inprogress→new axis. On the mirror that reported the bug, 59% of real
// reopens were 'QA testing → Reopened' moves on a workflow whose resolved
// states sit in the inprogress category — rows the done-only rule stored as
// reopen_count = 0. The work is backfillFlow's (flow.go), the same single
// owner the v43 migration and the snapshot pipeline use, so a migrated
// mirror and a freshly synced one cannot disagree. The SELECT is a no-op
// statement the migration runner can exec: a migration with no DDL still has
// to advance user_version, which only running an entry does.
const schemaV49 = `
SELECT 1;
`
