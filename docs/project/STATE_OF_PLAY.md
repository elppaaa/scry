...
Atlassian account and stops needing a Mac: the built-in tracker, whose origin
...
- `tools/seed-demo` — Go port of the demo-site seeder (the Python original is
  gone).
- `internal/mcp` — stdio JSON-RPC server, ten tools (four reads, `gadak_retro` for the retrospective document, `gadak_sprint` for one sprint's daily burn-up, `gadak_recents` for this machine's read trail (which `gadak_issue` and `gadak_search` append to), `gadak_show` for presentation, and the `gadak_ui_tokens` / `gadak_ui_set` design-token pair that writes `config.json` only), no writes to the mirror or to Jira, no SDK.
...
Development-panel pull requests live in `dev_links`: the built-in tracker
writes them through `gadak dev link` / `gadak dev scan` (the latter shells
out to user-invoked `gh`); a Jira Cloud workspace mirrors Jira's
