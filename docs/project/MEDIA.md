...
| `docs/media/sprint-retro-hero{,.ko,.ja}.mp4` (+ the matching `.gif` and `sprint-retro-hero-poster{,.ko,.ja}.png`) | Playwright `e2e/demo/sprint-retro-hero.spec.ts` via `e2e/demo/export-sprint-retro-hero.sh` (`make media-hero-sprint-retro` — outside the `make media` aggregate, same reason as media-sprint) | the 0.22 release clip, all three locales (ko for Twitter, en for HN·GitHub, ja for Qiita·Zenn) — both halves in one take: the board scoped to the active sprint with its strip (name · goal · dates · days left · progress bar, the Done column full beside it), then the retro from the palette with its summary strip and sparklines, cut by sprint, and one number opened into its issues |
...
GADAK_MEDIA_LOCALE=ko make media-sprint   # → sprint.ko.mp4,  sprint.ko.gif,  sprint-poster.ko.png
GADAK_MEDIA_LOCALE=ko make media-retro    # → retro.ko.mp4,   retro.ko.gif,   retro-poster.ko.png
GADAK_MEDIA_LOCALE=ko make media-hero-sprint-retro   # → sprint-retro-hero.ko.mp4, .gif, sprint-retro-hero-poster.ko.png (the release clip; ja and unset=en are the other two takes)
...
| `sprint-retro-hero{,.ko,.ja}.gif` | **≤ 4 MB**, enforced | four rungs (fps, then colors, then width) and then `gifsicle -O3 --colors 64`; `export-sprint-retro-hero.sh` exits 4 rather than write an over-budget gif. ja needs the fourth rung (760 px @ 6 fps) that en and ko do not — kanji carry more detail per frame (2026-09-13: 4.43 MB at rung three, 3.70 MB at rung four) |
| `sprint-retro-hero{,.ko,.ja}.mp4` | soft ≤ 8 MB | 1280×800 h264 `yuv420p` + `faststart`; ~1.7 MB at 16 s |
...
| --- | --- | --- | --- | --- |
| `scale.mp4` | 3.80 MB | 3797221 | 16.9 s | 1280×800 h264 |
| `scale.ko.mp4` | 3.02 MB | 3023050 | 16.6 s | 1280×800 h264 |
| `scale.ja.mp4` | 3.63 MB | 3628415 | 16.8 s | 1280×800 h264 |
| `web-demo.gif` | 8.04 MB | 8041523 | 17.6 s | 960×600 @ 9 fps |
| `web-demo.mp4` | 1.12 MB | 1119100 | 17.5 s | 1024×640 h264 |
| `search.gif` | 4.00 MB | 4004308 | 7.5 s | 960×600 @ 9 fps |
| `search.ko.gif` | 3.59 MB | 3594512 | 7.0 s | 960×600 @ 9 fps |
| `search.ja.gif` | 3.69 MB | 3688549 | 6.7 s | 960×600 @ 9 fps |
| `search.mp4` | 0.59 MB | 590375 | 7.6 s | 800×640 h264 |
| `search.ko.mp4` | 0.52 MB | 518846 | 7.0 s | 800×640 h264 |
| `search.ja.mp4` | 0.54 MB | 542203 | 6.7 s | 800×640 h264 |
...
| `scale.gif` | 2.97 MB | 2972888 | 16.9 s | 800×500 @ 7 fps |
| `scale.ko.gif` | 3.76 MB | 3762893 | 16.5 s | 960×600 @ 8 fps |
| `scale.ja.gif` | 3.11 MB | 3113263 | 16.9 s | 800×500 @ 7 fps |
...
make media-sprint  # Playwright: board scope active → backlog → all → sprint.gif + sprint.mp4 (GADAK_MEDIA_LOCALE=ko|ja)
make media-retro   # Playwright: ⌘K weekly retro → a cell opens its issues → retro.gif + retro.mp4 (GADAK_MEDIA_LOCALE=ko|ja)
make media-agent   # Playwright split: sql \| views open --keys + paper list → gif + mp4 (rig kept; the asset retired for terminal-demo)
make media-hero-sprint-retro   # Playwright: sprint strip + carried card → ⌘K retro summary → sprint cut → a cell opens its issues (GADAK_MEDIA_LOCALE=ko|ja)
