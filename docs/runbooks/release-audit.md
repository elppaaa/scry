...
| Go slowest | `go test -race -timeout 30m -count=1 -json ./... > /tmp/gotest.json && python3 tools/slowest -n 20 /tmp/gotest.json` (the only heavy command; nothing else runs beside it. `-timeout 30m` is load-bearing: cmd/gadak alone overruns -race's 10m default — measured timing out at exactly 10m0s on a quiet machine, 2026-09-11, so the census row for it is a panic dump, not timings, and the command exits nonzero. `tools/slowest` is a Python script — `python3 tools/slowest`, never `go run`. **The table is a `-race` measurement and the page must say so**: CI races only `internal/server` and `internal/workspace` (`.github/workflows/ci.yml:180,183`), so every other row is local race cost, not CI seconds — measured 2026-09-15, `TestEditTypeResolvesLikeCreate` 8.24 s raced against 0.44 s un-raced. An axis reading this table for CI minutes re-measures un-raced first) | axis 5 |
...
| Crash mid-migration | the migration runner — one transaction per migration, `user_version` written inside it | a kill between two migrations leaves the file at the previous stamp with nothing half-applied; a migration that does two file commits (the v26 shape — the cross-file `INSERT OR IGNORE INTO local.*` block, `schema.go:499`) is named in the report |
| Time on a real mirror | a copy of the largest local mirror opened at head, timed — measure, do not assume which one it is (2026-09-15: the root profile at 624 MB / 7,177 issues is the largest, and the smaller `work` mirror was the *slower* open of the two) | the first open finishes in seconds, and the number is in the report — a migration that rewrites `items_fts` or every changelog row is the kind that turns a first launch into a hang |
