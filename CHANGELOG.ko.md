...
- **macOS에서 ⌘W가 나머지 절반을 갖습니다.** 이 액셀러레이터는 보이는 인앱
  브라우즈 탭을 닫았고, 탭이 없으면 할 일이 없었습니다 — 앱 안에서 Atlassian
  링크를 열기 전까지의 모든 순간이 그랬습니다. 이제 `Window > Close`입니다:
  탭이 있으면 탭, 없으면 `[NSApp hide:]`. 아무것도 파괴하지 않으므로 창은 같은
  스크롤 위치, 열려 있던 다이얼로그, 쓰다 만 댓글을 그대로 갖고 돌아오며
  ⌘Tab과 Dock 양쪽에서 복귀합니다. 진짜 닫기가 아닌 이유는 그 창이 마지막
  창이라 앱이 종료되기 때문입니다. 항목이 동작할 수 없는 곳에서 제거한
  [GDK-351]을 이어받아, 동작할 수 있는 곳에 할 일을 줍니다.
  `docs/decisions/0011`.
