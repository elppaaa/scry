...
/**
 * Builds request headers — exported for tests. Bearer only when a token
...
interface RequestOpts {
  ...
  session?: ApiSession
  /**
   * A multipart body, the one kind that is not JSON (GDK-1872: attach a photo
   * from the field). Mutually exclusive with `body`. It is handed to the
   * runtime untouched and NO Content-Type is set, because the boundary is
   * generated by whichever fetch answers: the WebView's own, or
   * plugin-http's, which builds a `new Request(input, init)` and copies the
   * header that Request stamped only when the caller declared none
   * (@tauri-apps/plugin-http/dist-js/index.js:66-75). Declaring one here
   * would send a boundary-less multipart type and the server's FormFile
   * would refuse it.
   */
  form?: FormData
...
async function dial(path: string, opts: RequestOpts): Promise<Response> {
  // Two bodies is a programming error, not a server verdict: refused here,
  // before the scope check and before a fetch is even chosen, so no caller
  // can ship a request whose Content-Type contradicts its bytes.
  if (opts.body !== undefined && opts.form !== undefined) {
    throw new ApiError('bad_request', 0)
  }
  ...
  try {
    res = await doFetch(url, {
      ...
      headers,
      // The FormData goes through untouched — serializing it is the
      // runtime's job, and so is the Content-Type that names its boundary.
