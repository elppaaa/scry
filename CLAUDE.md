...
- **CHANGELOG는 릴리스마다 주제 두세 개다 — 사건의 나열이 아니다** (사용자 지시
  2026-09-09 "개발과정에서 사고치고 수습하고 단계별로 수정하고가 아니라 이전 버전
  대비 달라진 핵심이 잡혀야 하는데 자꾸 사건들의 나열이 된다"). 한 릴리스 절은
  **굵은 머리로 시작하는 주제 문단 최대 셋**이고, 각 문단은 직전 버전 대비 무엇이
  달라졌는지를 말하며 그 주제에 속하는 `GDK-nnn`을 문장 안에 인용한다. 굵은
  머리 없는 문단은 바로 위 주제의 연속이다. **최상위 불릿(`- `)은 금지** — 그게
  사고 일지의 모양이다. 재는 것은 `tools/doc-checks.sh` #46(세 판본 전부).
  Unreleased에 항목을 더할 때도 새 불릿을 붙이지 말고 있는 주제 문단에 문장을
  넣거나, 셋째 주제가 정말 새로 생겼을 때만 문단을 연다.
- **CHANGELOG는 다시 쓸 수 있다 — 서사를 위해서라면 과감하게 정리한다**
  (사용자 지시 2026-09-09). 항목을 합치고, 순서를 바꾸고, 줄이고, 키 없는 항목은
  지워도 된다. 불변은 셋이다: ① **릴리스 헤딩과 그 순서**(사이트가 그것으로
  점프 목록을 만들고 doc-checks #27이 세 판본을 대조한다) ② **살아남은 항목의
  숫자·명령·경로·플래그·코드·링크** ③ 없던 일을 새로 쓰지 않는다.
  **키는 불변이 아니다** (사용자 결정 2026-09-13): 독자가 못 느끼는 항목은
  `GDK-nnn`째로 지운다 — 0.22.0 이 그렇게 345키 4,983단어에서 19키 479단어가
  됐고, 감사·리팩토링 라운드 문단이 통째로 빠졌다. 판단은 "이걸 gadak 여는
  사람이 알아채는가" 하나이고, 지운 키의 참조 정의도 같이 빠진다
  (`python3 tools/changelog-tails.py --write <파일>`). 재는 것은
  `bash tools/changelog-preserve.sh <파일> [ref]` — 헤딩이 움직이면 진짜
  실패이고, **키·숫자·코드 스팬 축은 항목을 지운 라운드에서 정당하게 빨갛다**
  (그 출력을 읽고 무엇을 왜 지웠는지 커밋 메시지에 적는다). 판본은 셋이고(`CHANGELOG.md`·`.ko.md`·`.ja.md`) 여기서는
  병렬 판본이 아니라 같은 역사의 번역이다 — **영문 구조를 바꾸면 ko·ja도 같은
...
- **판단 기준은 서사다** (사용자 지시 2026-09-12: "제일 중요한건 서사야. 일관된
  방향으로 얼라인을 하려고 하고 이게 홍보하기 좋게 내러티브가 짜여져야해").
  이슈를 열고, 닫고, 순번을 매기는 기준은 기술적 완성도가 아니라 **그 결정이
  다음 릴리스 CHANGELOG의 주제 문단 어디에 들어가는가**다. 들어갈 문장이 없으면
  만들지 않고, 이미 있어도 폐기한다 — "있으면 좋은 것"은 서사 밖이다. CHANGELOG
  규칙(릴리스마다 주제 두세 개·불릿 금지)이 이 기준의 실측 게이트인 이유가
  이것이다: 문단에 안 들어가는 항목은 문단이 모자라서가 아니라 서사에 없어서
  안 들어간다. 서사 원본은 `docs/project/THEORY.md`와 제품 테제 삼단(핵심=셀프호스트
  PM, 엔트리=Jira·Linear 캐시, 에이전트=따라오는 것)이다.
  **판단이 갈리면 어드바이저에게 묻고**(같은 사용자 지시), 채택한 근거를 이슈
  코멘트에 한 문장으로 남긴다 — 나중에 그 문장이 CHANGELOG 문단의 초고가 된다.
  위임 스펙에는 넣지 않는다: 서사 판정은 리드 몫이고, 위임자에게 시키면 없는
  문장을 지어낸다.
- **등재·소개 문구는 그 채널이 설치해 주는 표면을 설명한다** (사용자 결정
  2026-09-13). 캐시와 빌트인 트래커 중 하나를 전역으로 고르지 않는다. MCP 채널
  (MCP 목록·디렉터리·`.mcpb`·Registry)은 툴이 전부 읽기라서(`docs/MCP.md`) 캐시
  조회를 앞에 두고 빌트인 트래커는 한 구절로 둔다 — "에이전트용 트래커"를 앞세우면
  설치한 사람이 쓰기 툴이 없다는 것을 먼저 발견한다. 빌트인 트래커를 앞세우는
  자리는 셀프호스트·CLI·로컬 퍼스트 목록이다. MCP 채널에서 빌트인을 앞세우려면
  MCP 쓰기 툴이 먼저이고, 그건 문구가 아니라 설계 결정이다.
...
- **GitHub(이슈·PR)에 남기는 코멘트는 첫 줄에 AI가 작성했음을 밝히고 건조하게
  쓴다** (사용자 지시 2026-09-15). 감탄·이모지·감사 인사 없이 사실(무엇을
  했고, 무엇을 확인했고, 어디에 기록됐는지)만 적는다. 기여자에게 보내는 말은
  사용자의 몫이다.
